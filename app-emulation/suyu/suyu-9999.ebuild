...
IUSE="+compatibility-list +cubeb discord +qt6 sdl +system-vulkan test webengine +webservice"
...
"
REQUIRED_USE="|| ( qt6 sdl ) discord? ( webservice )"
RESTRICT="!test? ( test )"
...
src_configure() {
	local -a mycmakeargs=(
		...
		-DENABLE_LIBUSB=ON
		-DENABLE_QT=$(usev qt6 ON || echo OFF )
		-DENABLE_QT_TRANSLATION=$(usev qt6 ON || echo OFF )
		...
		-DSUYU_USE_EXTERNAL_VULKAN_HEADERS=$(usex system-vulkan no yes)
		-DSUYU_USE_EXTERNAL_VULKAN_UTILITY_LIBRARIES=$(usex system-vulkan no yes)
